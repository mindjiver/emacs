...
;;; Modified by Bill Sommerfeld <sommerfeld@apollo.hp.com> to 
;;; support ClearCase
...
(defvar vc-cc-status t
  "*If non-nil, revision and locks on ClearCase working file displayed in modeline.
Otherwise, not displayed.")

;; Tell Emacs about this new kind of minor mode
(if (not (assoc 'vc-mode minor-mode-alist))
    (setq minor-mode-alist (append
			    minor-mode-alist  '((vc-mode vc-mode)))))
  ...
  (let (handler handlers)
	...
	(funcall handler 'vc-registered file)
      ...
      (let ((dirname (or (file-name-directory file) ""))
	    ...
	    (file-is-directory (file-directory-p file)))
	(catch 'found
	  (mapcar
	   (function 
	    (lambda (s)
	      (let* ((trial 
		      (format (if (listp (car s))
				  (car (car s))
				  (car s))
			      dirname basename))
		     (backend (if (listp (car s))
				  (format (cdr (car s)) dirname basename)
				trial))
		     (backname (cdr s)))
		(if (and (or (eq backname '@@)
			     (not file-is-directory))
			 (file-exists-p trial)
		    ...
		    (throw 'found (cons backend backname))))))
	   vc-master-templates)
	  nil)))))
...
of the buffer."
  ...
  (let ((fn (buffer-file-name)))
    (cond ((and fn (vc-backend-deduce fn))
	   (vc-next-action nil))
	  ((and (eq major-mode 'dired-mode)
		(vc-backend-deduce default-directory))
	   (vc-next-dir-action nil))
	  (t 
	   (toggle-read-only)))))
	      ...
	      (and vc-type
		   (concat " " (or label (symbol-name vc-type))
			   (if (and vc-rcs-status (eq vc-type 'RCS))
			       (vc-rcs-status file))
			   (if (and vc-cc-status (eq vc-type '@@))
			       (vc-cc-status file)))))
...
(defmacro vc-add-to-alist (set tag value)
  (list 'progn
	(list 'or (list 'assoc tag set)
	      (list 'setq set (list 'cons (list 'cons tag value)
			      set)))
	nil))

;;; XXX fix up doc-strings on these
(autoload 'vc-cc-view-handler "vc")
(autoload 'vc-cc-file-handler "vc")
(autoload 'vc-cc-browse-versions "vc")
(autoload 'vc-edit-config "vc" "Edit config spec" t)
(autoload 'vc-graph-history "vc" "View history graph" t)
(autoload 'vc-cc-what-rule "vc" "Show which rule selected this element" t)

(vc-add-to-alist file-name-handler-alist "^/view/[^/]+/" 'vc-cc-file-handler)
(vc-add-to-alist file-name-handler-alist "^/view[/]*$" 'vc-cc-view-handler)
